  ...
  return (
    <div className="relative mx-auto min-h-screen w-full max-w-[420px] bg-linen pb-24">
      ...
      >
        <button
          onClick={() => navigate(-1)}
          className="absolute left-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-lg"
        >
          ‹
        </button>
      ...
      <div className="px-5">
        <div className="-mt-8 rounded-card border border-[color:var(--line)] bg-white p-4 shadow-card">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
            <span className="rounded-full bg-linen px-2.5 py-1 text-ember-dark">
              ★ {cook.rating.toFixed(1)} ({cook.reviewCount})
            </span>
            <span className="rounded-full bg-linen px-2.5 py-1 text-[color:var(--muted)]">
              📍 {cook.city}
            </span>
            <span className="rounded-full bg-linen px-2.5 py-1 text-[color:var(--muted)]">
      ...
      {conflict && (
        <div className="fixed inset-0 z-20 flex items-end justify-center bg-black/40" onClick={() => setConflict(null)}>
          <div
            className="w-full max-w-[420px] rounded-t-card bg-white p-5"
